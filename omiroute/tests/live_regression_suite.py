...
safe_print("  Logical Role: harshu-classifier")
safe_print("  Provider: groq")
...
safe_print("  Logical Role: harshu-judge")
safe_print("  Provider: groq")
...
safe_print("\n[3. GENERAL CHAT]")
safe_print("  Logical Role: harshu-general")
safe_print("  Provider: groq")
...
safe_print("\n[4. REASONING CALL]")
safe_print("  Logical Role: harshu-reasoning")
safe_print("  Provider: groq")
...
safe_print("\n[6. TOOL RESULT ROUND-TRIP]")
safe_print("  Logical Role: harshu-tools")
safe_print("  Provider: groq")
...
safe_print("\n[7. DETERMINISTIC FALLBACK]")
safe_print("  Logical Role: test-live-fallback")
safe_print("  Primary Configured: groq/nonexistent-dummy-model-p1 (Failed)")
safe_print("  Provider: groq")
safe_print(f"  Actual Model: {model_fb}")
safe_print(f"  HTTP Status: {r_fb.status_code}")
safe_print(f"  Latency: {round(dt_fb, 2)} ms")
safe_print("  Fallback: YES (Recovered from primary failure to secondary)")
...
safe_print("  Provider: gemini")
safe_print("  Actual Model: gemini-embedding-2")
safe_print(f"  HTTP Status: {r_emb.status_code}")
safe_print(f"  Latency: {round(dt_emb, 2)} ms")
safe_print(f"  Dimension: {vec_len}")
safe_print("  Fallback: NO (Strict Primary Only)")
